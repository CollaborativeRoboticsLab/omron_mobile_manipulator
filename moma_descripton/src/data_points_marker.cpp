#include "moma_descripton/data_points_marker.hpp"
...
{
  // Read map file
  std::string map_path = ament_index_cpp::get_package_share_directory("moma_descripton") + "/map/" + filename;
  ...
  while (std::getline(map_file, line))
  {
    ...
    if (line.find(POINTS_H) != std::string::npos)
    {
      ...
      for (int i = 0; i < num_of_points; i++)
      {
        ...
        if (!(iss >> x >> y))
        {
          RCLCPP_ERROR(this->get_logger(), "%s - Error reading points from file.", "moma_descripton");
        }
        set_point(x, y);
      }
    }
